...
output$xvar <- renderUI({
  ...
  spinners <- get_spinsters(graphtxt_buffer)
  base_choices <- sort(unique(setdiff(all_cols, union(orphans, spinners))))

  if (isTRUE(locked())) {
    # Freeze to canonical X and expose ONLY that choice
    selected_val <- isolate(vars_sel()$x)
    ...
    tagList(
      selectInput(
        ...
        choices = choices,          # no "" here when locked
      ...
      tags$script(HTML("
        ...
        var t = setInterval(function(){
          ...
          if ($sel.length && ($inp.length ? $inp.parent().hasClass('selectize-input') : true)) {
    ...
    # Unlocked: keep prior input if present; allow it even if not in base_choices
    selected_val <- isolate(input$xvar)
    if (is.null(selected_val)) selected_val <- ""
    choices <- if (nzchar(selected_val) && !(selected_val %in% base_choices)) {
      c(selected_val, base_choices)
    } else {
      base_choices
    }
    if (!(selected_val %in% c("", choices))) selected_val <- ""
...
output$yvar <- renderUI({
  ...
  if (isTRUE(locked())) {
    ...
    # Unlocked: keep prior input if present; allow it even if not in descs
    selected_val <- isolate(input$yvar)
    if (is.null(selected_val)) selected_val <- ""
    choices <- if (nzchar(selected_val) && !(selected_val %in% descs)) {
      c(selected_val, descs)
    } else {
      descs
    }
    if (!(selected_val %in% c("", choices))) selected_val <- ""
