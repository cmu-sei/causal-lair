...
log_file <- file.path(
  log_dir,
  sprintf("%s_error_log.txt", format(Sys.time(), "%Y-%m-%d_%H-%M-%S"))
)
...
nice_step <- function(span) {
  # span = max - min
  if (span <= 0 || is.na(span) || is.infinite(span)) return(1)

  # one order of magnitude below span, rounded to 1 sig-fig
  signif(span / 10, 1)
}
...
make_threshold_ui <- function(id, vec) {
  rng   <- range(vec, na.rm = TRUE)
  step  <- nice_step(diff(rng))
...
output$ui_threshold_x <- renderUI({
  ...
  tagList(
    tags$div(
      ...
      tags$span(paste(input$xvar,
                      "is considered treated when it is ≥")),
      make_threshold_ui("tv_threshold", df()[[input$xvar]])
    )
  )
})
...
output$ui_threshold_y <- renderUI({
  ...
  tagList(
    tags$div(
      style = "display:flex;align-items:center;flex-wrap:wrap;",
      tags$span(paste(input$yvar,
                      "is considered success when it is ≥")),
      make_threshold_ui("ov_threshold", df()[[input$yvar]])
    )
  )
})
...
output$ui_model_exist <- renderUI({
  ...
  selectInput(
    ...
    selected = "ATE",
    width    = "100%"          # stretches full column width
  )
})
