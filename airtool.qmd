...
runSuperLearner <- function(settings,
  ...
  cat(paste0(Sys.time(), " - ","Started superlearner with ",
             paste(c(settings, AIRHome, tv_threshold, ov_threshold), collapse = ", ")), "\n", 
      file = log_file, 
      append = TRUE)
...
processResults <- function(settings,
  ...
  log_msg("Settings:", paste(c(settings, AIRHome, tv_threshold, ov_threshold, model_yn, model_ate), collapse = ", "))
...
observeEvent( vars_ready(), ignoreInit = TRUE, {
  ...
  cat(sprintf("%s - Selected X threshold: %s\n", Sys.time(), input$tv_threshold), file = log_file, append = TRUE)
  cat(sprintf("%s - Selected Y threshold: %s\n", Sys.time(), input$ov_threshold), file = log_file, append = TRUE)
    ...
    withProgress(message = 'Building Causal Graph', style = "notification", value = 0.1, {
      ...
      tryCatch({
        ...
        results_list <- foreach(k = 1:nrow(combos),
          ...
          res <- tryCatch(
            runSuperLearner(settings,
                            ...
                            log_file),
            error = function(e) {
              cat(sprintf("SuperLearner: Error i=%s, j=%s: %s", i, j, conditionMessage(e)), file = log_file, sep = "\n", append = TRUE)
              NULL
            }
          )
          res
        ...
        tryCatch({
          ...
          cat(sprintf("processResults() failed: %s", conditionMessage(e)), file = log_file, append = TRUE)
        })
