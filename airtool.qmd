...
This suggests that the data or model assumptions may not support a consistent causal interpretation for this configuration.",
  ...
  p <- ggplot(dfr0, aes(x = Treatment)) +
    ...
    annotate("text",
             x = 1, y = -1,
             label = "-", hjust = 2.5, vjust = 0.25,
             size = 5, color = "white") +
    annotate("text",
             x = 1, y = 1,
             label = "+", hjust = -0.35, vjust = 0.4,
             size = 5, color = "white") +
...
output$ribbon_plot <- renderImage(
  {
    ...
    # canvas hugs the ribbon (aspect.ratio = 0.2 panel) -- a taller canvas
    # just bakes white bands above/below the strip into the PNG. The
    # algorithm-flag arrow (drawn when a model/ATE was supplied) sits well
    # above the strip via clip = "off", so it needs extra headroom
    has_flag <- !is.null(rv$model_yn) && rv$model_yn != "null"
    ggsave(tmp, plot = plt,
           width = 6.5, height = if (has_flag) 3.25 else 1.8,
