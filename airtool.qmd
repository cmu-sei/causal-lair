...
tagList(
  # 1) Normal Shiny button lives in the sidebar
  uiOutput("ui_reset_btn"),

  # 2–3) JS: switch between fixed-at-bottom (no scroll) and sticky (when scroll shows up)
  tags$script(HTML("
    (function () {
      const slot = document.getElementById('reload_slot');
      if (!slot) return;

      // placeholder preserves original position for 'sticky' mode
      const ph = document.createElement('span');
      ph.id = 'reload_slot_ph';
      slot.parentNode.insertBefore(ph, slot);

      function getSidebar() {
        // Quarto dashboard sidebar container
        return document.querySelector('.quarto-dashboard-sidebar')
            || document.querySelector('.sidebar')
            || document.querySelector('[class*=sidebar]');
      }

      function getScrollEl(sidebar) {
        // find the actual scrolling element inside the sidebar
        const nodes = [sidebar, ...sidebar.querySelectorAll('*')];
      ...
      function placeFixed() {
        ...
        // move slot to body so overflow clipping won't affect it
      ...
      function placeSticky() {
        // move slot back under the placeholder, full-width of sidebar content
        if (slot.parentNode !== ph.parentNode) {
          ph.parentNode.insertBefore(slot, ph.nextSibling);
        }
        Object.assign(slot.style, {
          position: 'sticky',
          bottom: '12px',
      ...
      function refreshPlacement() {
        const sb = getSidebar();
        if (!sb) return;
        const scroller = getScrollEl(sb);
        const needsScroll = scroller.scrollHeight > scroller.clientHeight + 1;
        if (needsScroll) placeSticky(); else placeFixed();
      }

      // run now and on likely changes
      setTimeout(refreshPlacement, 0);
      window.addEventListener('resize', refreshPlacement);
      document.addEventListener('shiny:value', refreshPlacement);
      document.addEventListener('shiny:outputinvalidated', refreshPlacement);

      const sb = getSidebar();
      if (sb) {
        new MutationObserver(refreshPlacement)
    ...
    tags$script(HTML(sprintf("
      (function(){
        ...
        function applyXDisabled(){
          ...
          if (sel.value && blocked.indexOf(sel.value) !== -1) {
            sel.value = '';
            $(sel).trigger('change');
          }
        ...
        $(document).on('shiny:value.xvarBlock shiny:visualchange.xvarBlock', applyXDisabled);
      })();
    ...
    tagList(
      ...
      tags$script(HTML(sprintf("
        (function(){
          ...
          $(document).on('shiny:value.yvarBlock shiny:visualchange.yvarBlock', applyYDisabled);
        })();
