...
AIR_getGraph <- function(data, knowledge, graphtxt_buffer = NULL) {
  ...
  graph_str <- tryCatch(
    ...
    error = function(e) {
      log_debug(sprintf("Failed to convert graph to string: %s", conditionMessage(e)))
      NULL
    }
  )
...
observeEvent( vars_ready(), ignoreInit = TRUE, {
  ## only run after the second selector is filled in
  req(vars_ready())
  ...
  Zvars_loc <- rbind(
    data.frame(name = input$xvar, grp = "Z1", Z = Z1),
    data.frame(name = input$xvar, grp = "Z2", Z = Z2)
  )
  assign("Zvars", Zvars_loc, envir = .GlobalEnv)

  ## ---------- 2. logging (unchanged) ---------------------------------------
  log_debug("Graph Updated.")
  log_debug("Selected X variable: {msg}", msg = input$xvar)
  log_debug("Selected Y variable: {msg}", msg = input$yvar)
  log_debug("Selected X threshold: {msg}", msg = input$tv_threshold)
  log_debug("Selected Y threshold: {msg}", msg = input$ov_threshold)
  ...
  tryCatch({
    ...
    withProgress(message = 'Building Causal Graph', style = "notification", value = 0.1, {
      ...
      tryCatch({
        ...
        clusterExport(cluster, "df_vars", envir = .GlobalEnv)
...
output$xvar = renderUI({
  req(graph_complete())
  selected_val <- if (locked()) isolate(input$xvar) else ""
  tagList(
...
output$yvar = renderUI({
  req(input$xvar)
  choices <- setdiff(get_X_descendents(input$xvar, AIRHome), input$xvar)
  selected_val <- if (locked()) isolate(input$yvar) else ""
  tagList(
...
fine_step <- function(vec, n_bins = 30) {
 ...
 # vec = df()[[input$xvar]], step = fine_step(vec)
...
output$ui_threshold_x <- renderUI({
  req(input$xvar)
  vec <- df()[[input$xvar]]
  val <- if (locked()) isolate(input$tv_threshold) else default_split(vec)
  step <- fine_step(vec)  # half a bin, n_bins = 30
  tagList(
    tags$div(
      style = "display:flex;align-items:center;flex-wrap:wrap;",
      tags$span(paste(input$xvar, "is considered treated when it is ≥")),
...
output$ui_threshold_y <- renderUI({
  req(input$yvar)
  vec <- df()[[input$yvar]]
  val <- if (locked()) isolate(input$ov_threshold) else default_split(vec)
  step <- fine_step(vec)  # half a bin, n_bins = 30
  tagList(
    tags$div(
      style = "display:flex;align-items:center;flex-wrap:wrap;",
      tags$span(paste(input$yvar, "is considered success when it is ≥")),
...
output$histogram_x <- renderPlot({
  req(input$xvar, input$tv_threshold)
  p <- get_histogram_x(df(), input$xvar, input$tv_threshold)
  p
  }, bg = "transparent")

output$histogram_y <- renderPlot({
    req(input$yvar, input$ov_threshold)
  p <- get_histogram_y(df(), input$yvar, input$ov_threshold)
  p
  }, bg = "transparent")
  ...
  content = function(file) {
    tryCatch({
      ...
      vals <- isolate(list(
        ...
        yvar          = input$yvar,
        df            = df(),
        tv_threshold  = input$tv_threshold,
        ov_threshold  = input$ov_threshold,
        Zvars         = Zvars,
