...
options(shiny.maxRequestSize = Inf) # 200 * 1024^2) # sets max upload to 200MB
...
get_updated_graph <- function(dot, xvar, yvar, Zvars = NULL) {
  ...
  new_lines <- c(
    if (nzchar(xvar)) sapply(xvar, make_decl, colour = "#FFC107"),
    if (nzchar(yvar)) sapply(yvar, make_decl, colour = "#FFC107"),
...
output$download_selected <- downloadHandler(
  ...
  content = function(file) {
    ...
    shiny::validate(shiny::need(isTRUE(ok), "Failed to copy log to download location."))
  }
) 
...
df <- reactive({
  ...
  shiny::validate(
    ...
    shiny::need(file.size(input$file1$datapath) < 500*1024^2,
                "CSV too large (500 MB limit)")
  )
...
output$xvar <- renderUI({
  ...
  # compute allowed X choices
  all_cols <- names(df())
  orphans  <- get_orphans(graphtxt_buffer)   # nodes without parents
  spinsters <- get_spinsters(graphtxt_buffer) # nodes without children
  base_choices <- sort(unique(setdiff(all_cols, union(orphans, spinsters))))
  blocked <- sort(setdiff(all_cols, base_choices))  # visible but not selectable
  ...
  # keep prior selection only if it's still allowed
selected_val <- {
  prev <- isolate(input$xvar)
  if (!is.null(prev) && prev %in% base_choices) prev else ""
}

# UI: native <select>, grouped, with blocked options disabled via JS
tagList(
  selectInput(
    inputId   = "xvar",
    label     = "Experimental (X) variable:",
    # Group for clarity: users see both groups
    choices   = list(
      "Available"      = base_choices,
      "Not selectable" = blocked
    ),
    selected  = selected_val,
    selectize = FALSE   # IMPORTANT: native select supports per-option disabled
  ),

  # (Optional) visual nudge for disabled options
  tags$style(HTML("#xvar option:disabled { color:#9aa0a6; }")),

  # Disable the blocked options (and clear selection if it was blocked)
  # Uses jsonlite to safely embed the R vector in JS.
  tags$script(HTML(sprintf("
    (function(){
      function applyDisabled(){
        ...
        // If currently selected value is blocked, clear it
        if (blocked.indexOf(sel.value) !== -1) {
          sel.value = '';
          $(sel).trigger('change');
        }

        // Disable each blocked option (still visible, but not selectable)
        $('#xvar option').each(function(){
          if (blocked.indexOf(this.value) !== -1){
            this.disabled = true;
      ...
      // Run now and after Shiny updates
...
# In unlocked mode, require at least one descendant
if (!isTRUE(locked())) {
  shiny::validate(
...
# We'll show *all* columns but only allow selecting descendants
all_cols <- names(df())
blocked  <- sort(setdiff(all_cols, descs))  # visible but not selectable

if (isTRUE(locked())) {
  # Freeze to canonical Y and expose ONLY that choice
  selected_val <- isolate(vars_sel()$y)
  if (is.null(selected_val)) selected_val <- ""
  choices <- if (nzchar(selected_val)) selected_val else c("")
  ...
  # Unlocked: keep prior input if it's still valid; otherwise pick none (lets browser use first available)
  prev <- isolate(input$yvar)
  selected_val <- if (!is.null(prev) && prev %in% descs) prev else NULL

  # Grouped choices: users see both groups
  # (Put Available first so the first option is selectable even if selected_val is NULL)
  grouped_choices <- list(
    "Available (descendants of X)" = descs,
    "Not selectable"               = blocked
  )
  ...
  tagList(
    ...
    # Subtle gray for disabled options
    tags$style(HTML("#yvar option:disabled { color:#9aa0a6; }")),

    # Disable non-descendant options (still visible, but unselectable)
    tags$script(HTML(sprintf("
      (function(){
        function applyDisabled(){
          ...
          // If the currently selected value is blocked (shouldn't happen with selected_val logic),
          // clear it to let the user pick a valid descendant.
          if (blocked.indexOf(sel.value) !== -1) {
            sel.value = '';
            $(sel).trigger('change');
          }

          // Disable each blocked option
