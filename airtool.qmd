...
safe_jcall <- function(...) {
  tryCatch(
    .jcall(...),
    Throwable = function(e) {
      # Try to get the actual Java exception object for a full message
      java_ex <- tryCatch(.jgetEx(clear = TRUE), error = function(x) NULL)
      msg <- if (!is.null(java_ex)) {
        tryCatch(
          .jcall(java_ex, "S", "toString"),   # instance call — this is correct
          error = function(x) conditionMessage(e)
        )
      } else {
        conditionMessage(e)
      }
...
escape_dot <- function(x) gsub('[^\\w\\s:]', '_', x, perl = TRUE)  # DOT-safe; : is literal inside quoted node IDs
...
# fontcolor is optional -- pass it for dark fills (e.g. the red bias flag)
# where the default black label text is hard to read
change_node_color <- function(dot_code, node, color, fontcolor = NULL) {
  node <- escape_dot(node)
  # Remove any accidental extra quotes from the color string
  color <- trimws(gsub("['\"]", "", color))
  attrs <- paste0('style=filled, fillcolor="', color, '"')
  if (!is.null(fontcolor)) {
    fontcolor <- trimws(gsub("['\"]", "", fontcolor))
    attrs <- paste0(attrs, ', fontcolor="', fontcolor, '"')
  }
  node_definition <- paste0("\"", node, "\" [", attrs, "];")
...
paint_nodes <- function(dot, nodes, col, fontcolor = NULL) {
  nodes <- nodes[nzchar(nodes)]
  for (n in nodes) {
    dot <- change_node_color(dot, n, col, fontcolor)
  }
  dot
}
...
# log_debug(paste(capture.output(dplyr::glimpse(np_df)), collapse = "\n")) 
  ...
  for (i in seq(0, 15)) {
    ...
    tryCatch({
      ...
      if (r_ts_select()) time_lag_val <- 1 else time_lag_val <- 0
      suppress_output(ts$run_boss(is_time_series = r_ts_select(), time_lag = time_lag_val))
...
AIR_getAdjSets <- function(ts, tv, ov, MC_passing_cpdag_already_found, best_cpdag_seen_so_far){
  ...
  log_debug("Number of undirected edges found: {ue}", ue = undirected_edges)
    ...
    log_debug("adj_sets_treatment type: {type_t}, length: {len_t}",
          type_t = typeof(adj_sets_treatment), len_t = length(adj_sets_treatment))
    log_debug("adj_sets_response type: {type_r}, length: {len_r}",
          type_r = typeof(adj_sets_response), len_r = length(adj_sets_response))
  ...
  for (i in seq_along(adj_sets_treatment)) {
       set <- adj_sets_treatment[[i]]
      
       if (length(set) != 0) {
        len_adj_sets_t <- len_adj_sets_t + 1
       }
   }
   log_debug("Length of non-zero adjustment sets for treatment {set}", set = len_adj_sets_t)

    

  for (i in seq_along(adj_sets_response)) {
       set <- adj_sets_response[[i]]
      
       if (length(set) != 0) {
        len_adj_sets_r <- len_adj_sets_r + 1
       }
   }
   log_debug("Length of non-zero adjustment sets for response {set}", set = len_adj_sets_r)  
  ...
  log_debug("Size of Response - Treatment adjustment sets = {msg}", msg = length(near_response_not_near_treatment))
   ...
   if (MC_passing_cpdag_already_found == FALSE) {
    ...
    log_debug("*** WARNING: Markov Condition not satisfied for current graph! Proceed with caution. ***")
  } 
 
  
  log_debug("union of two lists = {union}", union = paste(capture.output(str(union_of_two_lists)), collapse = "\n"))
  log_debug("target adjustment sets: {tar}", tar = len_adj_sets_t)
  log_debug("response adjustment sets: {res}", res = len_adj_sets_r)
  ...
  if (length(union_of_two_lists) == 0 | (len_adj_sets_t + len_adj_sets_r == 0)) {
    ...
    if (undirected_edges == 0){
      log_debug("No adjustment set found. The structure of this data is not amenable to causal inference as no adjustment sets can be found.")
  
      flag_no_adjustment_set_found = TRUE    
      return(list("error","structure"))
    } else if (undirected_edges > 0){
...
processResults <- function(settings,
  ...
  # Drop columns that are constant within the training split: single-level
  # factors/characters cause "contrasts can be applied only to factors with 2
  # or more levels" in glm/lm/svm; zero-variance numerics cause singularity.
  const_cols <- names(train)[vapply(train, function(x) {
    if (is.factor(x) || is.character(x)) length(unique(x[!is.na(x)])) < 2
    else if (is.numeric(x) || is.logical(x)) length(unique(x[!is.na(x)])) < 2
    else FALSE
  }, logical(1L))]
  if (length(const_cols) > 0) {
    log_info("Dropping constant columns from ML training data: {cols}",
             cols = paste(const_cols, collapse = ", "))
    keep       <- setdiff(names(train),   const_cols)
    train      <- train[,   keep,                               drop = FALSE]
    xtest_0    <- xtest_0[, intersect(names(xtest_0), keep),   drop = FALSE]
    xtest_1    <- xtest_1[, intersect(names(xtest_1), keep),   drop = FALSE]
  }
  ...
  if (model_yn == "No") {
    ...
    classifier_buffer(ate_df_all)   # all 5 ATEs — used for spread display in interpretation
    ml_buffer(ate_df_ref)           # stacked-SL summary is the reference ATE
  ...
  # Abort only when *both* Z-level ATEs are completely absent — that means
  # every TMLE worker failed and there is nothing to display.  If only one
  # Z-level is missing (partial result) we log a warning and continue; the
  # ribbon plot and significance tests already guard individual NA columns.
  missing_vals <- purrr::keep(
    final_results[c("z1_ATE_LCI", "z1_ATE_UCI", "z2_ATE_LCI", "z2_ATE_UCI")],
    ~ all(is.na(.x))
  )
  z_missing <- intersect(names(missing_vals), c("z1_ATE_LCI", "z2_ATE_LCI"))
  if (length(z_missing) == 2 || all(is.na(final_results$Treatment))) {
    stop("processResults(): critical columns still NA – see log for details")
  }
  if (length(missing_vals) > 0) {
    log_info("⚠️ Partial results: {cols} are all NA — one adjustment-set level failed.",
             cols = paste(names(missing_vals), collapse = ", "))
  }
...
## 5. bold every edge whose source is one of `nodes` ─────────────────────
## (used to draw attention to edges leaving a bias-flagged/red node);
## if `only_into` is given, only edges whose TARGET is in it are bolded --
## used to restrict bolding to edges on the flagged Z → X/Y pathway
bold_edges_from <- function(dot, nodes, penwidth = 2, only_into = NULL) {
  nodes <- nodes[nzchar(nodes)]
  if (!length(nodes)) return(dot)

  lines <- strsplit(dot, "\n", fixed = TRUE)[[1]]

  bold_one <- function(line) {
    if (!is_edge_line(line) || !(first_node(line) %in% nodes)) return(line)
    if (!is.null(only_into) && !(second_node(line) %in% only_into)) return(line)

    if (grepl("penwidth\\s*=", line, perl = TRUE)) {
      sub("penwidth\\s*=\\s*[0-9.]+", paste0("penwidth=", penwidth), line, perl = TRUE)
    } else if (grepl("\\[", line, perl = TRUE)) {
      sub("\\[", paste0("[penwidth=", penwidth, ", "), line, perl = TRUE)
    } else {
      sub(";\\s*$", paste0(' [penwidth=', penwidth, '];'), line, perl = TRUE)
    }
  }

  paste(vapply(lines, bold_one, character(1), USE.NAMES = FALSE), collapse = "\n")
}

## 6. every node name appearing anywhere in a dot string (edges + standalone
## declarations) -- used to find the "everything else" set to gray out
all_dot_nodes <- function(dot) {
  lines <- strsplit(dot, "\n", fixed = TRUE)[[1]]

  is_decl_line <- function(line) {
    grepl('^\\s*"[^"]+"\\s*(\\[|;)', line, perl = TRUE) && !is_edge_line(line)
  }

  edge_lines <- lines[vapply(lines, is_edge_line, logical(1))]
  decl_lines <- lines[vapply(lines, is_decl_line, logical(1))]

  from_edges <- if (length(edge_lines))
    c(vapply(edge_lines, first_node,  character(1), USE.NAMES = FALSE),
      vapply(edge_lines, second_node, character(1), USE.NAMES = FALSE))
  else character(0)

  from_decls <- if (length(decl_lines))
    vapply(decl_lines, function(l) split_line(l)[1], character(1), USE.NAMES = FALSE)
  else character(0)

  unique(c(from_edges, from_decls))
}

## mute a node's outline + label only -- leaves fill untouched (unfilled/
## transparent), unlike change_node_color which always sets style=filled
mute_node <- function(dot_code, node, color, fontcolor) {
  node <- escape_dot(node)
  color <- trimws(gsub("['\"]", "", color))
  fontcolor <- trimws(gsub("['\"]", "", fontcolor))
  node_definition <- paste0("\"", node, "\" [color=\"", color, "\", fontcolor=\"", fontcolor, "\"];")
  dot_code <- sub("digraph g \\{", paste0("digraph g {\n  ", node_definition), dot_code)
  dot_code <- gsub("\'", "\"", dot_code)
  return(dot_code)
}

mute_nodes <- function(dot, nodes, color, fontcolor) {
  nodes <- nodes[nzchar(nodes)]
  for (n in nodes) dot <- mute_node(dot, n, color, fontcolor)
  dot
}

## nodes reachable FROM `sources` (fwd) and nodes that can REACH `targets`
## (bwd), following only directed "->" edges in the dot string. A node lies on
## some sources → … → targets path iff it's in both sets; an edge u -> v lies
## on such a path iff u ∈ fwd and v ∈ bwd. Undirected edges are deliberately
## ignored -- they aren't unambiguously "downstream" and stay muted.
dot_path_sets <- function(dot, sources, targets) {
  lines <- strsplit(dot, "\n", fixed = TRUE)[[1]]
  edge_lines <- lines[vapply(lines, is_edge_line, logical(1))]
  if (!length(edge_lines)) return(list(fwd = character(0), bwd = character(0)))

  from <- vapply(edge_lines, first_node,  character(1), USE.NAMES = FALSE)
  to   <- vapply(edge_lines, second_node, character(1), USE.NAMES = FALSE)

  reach <- function(seeds, from, to) {
    seen <- unique(seeds[nzchar(seeds)])
    frontier <- seen
    while (length(frontier)) {
      nxt <- unique(to[from %in% frontier])
  ...
  list(
    fwd = reach(sources, from, to),   # downstream of the flagged set
    bwd = reach(targets, to, from)    # upstream of X / Y (edges reversed)
  )
}

## 7. mute every node/edge *not* in `highlighted` -- rather than literally
## graying the whole graph first and layering colors on top (repeated node
## statements merge with the FIRST call winning on conflicts -- see
## change_node_color), we paint the highlighted set as usual and gray only
## the complement, which needs no ordering trick at all.
## `path_fwd`/`path_bwd` (from dot_path_sets) optionally spare the bias
## pathway: nodes in BOTH sets, and edges from a fwd node to a bwd node, keep
## their standard black border/text so the flagged Z → X/Y routes stay legible
gray_out_background <- function(dot, highlighted, node_color = "#B9BEC7",
                                 node_font = "#6B7280", edge_color = "#C7CBD1",
                                 path_fwd = character(0), path_bwd = character(0)) {
  highlighted <- unique(highlighted[nzchar(highlighted)])
  on_path <- intersect(path_fwd, path_bwd)
  focused <- c(highlighted, on_path)   # nodes that keep their normal look

  others <- setdiff(all_dot_nodes(dot), focused)
  dot <- mute_nodes(dot, others, node_color, node_font)

  lines <- strsplit(dot, "\n", fixed = TRUE)[[1]]
  fade_edge <- function(line) {
    if (!is_edge_line(line)) return(line)
    if (first_node(line) %in% path_fwd && second_node(line) %in% path_bwd)
      return(line)                                       # edge lies on a flagged Z → X/Y path
    if (first_node(line) %in% highlighted && second_node(line) %in% focused)
      return(line)                                       # between focused nodes (e.g. X → Y)

    if (grepl("\\[", line, perl = TRUE)) {
      sub("\\[", paste0('[color="', edge_color, '", '), line, perl = TRUE)
    } else {
      sub(";\\s*$", paste0(' [color="', edge_color, '"];'), line, perl = TRUE)
    }
  }
  paste(vapply(lines, fade_edge, character(1), USE.NAMES = FALSE), collapse = "\n")
}
...
get_ribbon_plot <- function(AIRHome) {
  ...
  overlap_exists <- hi_overlap >= lo_overlap && !any(is.na(c(lo_overlap, hi_overlap)))

  # --- Handle no overlap ---
  if (!overlap_exists) {
    sendSweetAlert(
      session = shiny::getDefaultReactiveDomain(),
      title = "Consensus could not be reached for this scenario",
      text = "The causal estimates derived from your two models (Z₁ and Z₂) do not agree within their confidence ranges.
This suggests that the data or model assumptions may not support a consistent causal interpretation for this configuration.",
      type = "error",
      btn_labels = "Continue",
  ...
  p <- ggplot(dfr0, aes(x = Treatment)) +
    ...
    ## +/- annotations, centered on the black end caps (anchored at the
    ## bar's endpoints so they stay put at any render size)
    annotate("text",
             x = 1, y = -1.05,
             label = "−", hjust = 0.5, vjust = 0.4,
             size = 5, color = "white") +
    annotate("text",
             x = 1, y = 1.05,
             label = "+", hjust = 0.5, vjust = 0.4,
    ...
    ## red baseline ribbon (everything outside CIs)
    ...
    ## yellow ribbons for Z1 and Z2 CIs
    ...
    ## conditional green overlap (only if overlap exists)
    {if (overlap_exists)
    ...
    geom_linerange(aes(ymin = z1_ATE_LCI, ymax = z1_ATE_UCI),
                   lwd = 3.5, col = "#8A93A6",
                   alpha = 1, stat = "unique",
                   lineend = "round", position = position_nudge(x = 1)) +
    geom_point(aes(y = z1_ATE),
               col = "white", cex = 3, pch = 1, stroke = 1.25,
    ...
    geom_linerange(aes(ymin = z2_ATE_LCI, ymax = z2_ATE_UCI),
                   lwd = 3.5, col = "#D6DAE3",
                   alpha = 1, stat = "unique",
                   lineend = "round", position = position_nudge(x = 0.5)) +
  ...
  if (rv$model_yn != "null") {
     p +
      ## arrow for algorithm flag -- below the strip, pointing up at the flag
    geom_segment(data = dfr,
                 aes(x = -0.5, xend = 0.75, y = flag, yend = flag, color = algorithm),
                 arrow = arrow(length = unit(0.5, "cm")),
                 lwd = 1.2,
                 color = "#0F9ED5") +
    geom_point(data = dfr,
               aes(x = -0.5, y = flag, shape = algorithm),
               size = 3, color = "#0F9ED5")
  } else {
    p
  }
...
get_ui_interpretation <- function(AIRHome, df_vars, Zvars, model_yn, n_obs) {
  dfr <- results_out_buffer()
  ...
  fmt_pct_signed <- function(x) {
    v <- round(x * 100, 0)
    if (v > 0) paste0("+", v, "%") else paste0(v, "%")
  }
  fmt_range <- function(lo, hi) paste0(fmt_pct_signed(lo), " to ", fmt_pct_signed(hi))
  fmt_range_abs <- function(lo, hi) {
    paste0(round(min(abs(lo), abs(hi)) * 100, 0), "\u2013",
           round(max(abs(lo), abs(hi)) * 100, 0), "%")
  }
  ...
  lo_overlap <- nz_max(c(dfr$z1_ATE_LCI, dfr$z2_ATE_LCI))
  hi_overlap <- nz_min(c(dfr$z1_ATE_UCI, dfr$z2_ATE_UCI))
  overlap_exists <- !is.na(lo_overlap) && !is.na(hi_overlap) && hi_overlap >= lo_overlap

  # ── CI width note ──────────────────────────────────────────────────────────
  z1_width   <- dfr$z1_ATE_UCI[1] - dfr$z1_ATE_LCI[1]
  z2_width   <- dfr$z2_ATE_UCI[1] - dfr$z2_ATE_LCI[1]
  max_width  <- max(z1_width, z2_width, na.rm = TRUE)
  ci_width_note <- if (max_width < 0.10) {
    "The uncertainty range is narrow, so this is a fairly precise estimate \u2014 not just a directional guess."
  } else if (max_width < 0.25) {
    "The uncertainty range is moderate; the direction of the effect is reliable but the exact size is approximate."
  } else {
    paste0(
      "Note: the uncertainty range is wide (spanning more than ",
      round(max_width * 100, 0),
      " percentage points). The data supports only a rough directional conclusion here, not a precise effect size. ",
      "Collecting more data or refining the causal graph may narrow this range."
    )
  }

  # ── Z1/Z2 point estimate proximity ─────────────────────────────────────────
  z1_pt <- dfr$z1_ATE[1]
  z2_pt <- dfr$z2_ATE[1]
  pt_gap <- abs(z1_pt - z2_pt)
  proximity_note <- if (pt_gap < 0.05) {
    paste0(
      "Both independent analyses give nearly identical estimates ",
      "(Analysis A: ", fmt_range(dfr$z1_ATE_LCI[1], dfr$z1_ATE_UCI[1]),
      "; Analysis B: ", fmt_range(dfr$z2_ATE_LCI[1], dfr$z2_ATE_UCI[1]),
      "), which adds confidence to this finding."
    )
  } else if (pt_gap < 0.10) {
    paste0(
      "The two analyses give similar estimates ",
      "(Analysis A: ", fmt_range(dfr$z1_ATE_LCI[1], dfr$z1_ATE_UCI[1]),
      "; Analysis B: ", fmt_range(dfr$z2_ATE_LCI[1], dfr$z2_ATE_UCI[1]), ")."
    )
  } else {
    paste0(
      "The two analyses agree on direction but differ in magnitude ",
      "(Analysis A: ", fmt_range(dfr$z1_ATE_LCI[1], dfr$z1_ATE_UCI[1]),
      "; Analysis B: ", fmt_range(dfr$z2_ATE_LCI[1], dfr$z2_ATE_UCI[1]),
      "). Treat the exact size with some caution."
    )
  }

  # ── Effect direction (from consensus range, or full range if no overlap) ───
  if (overlap_exists) {
    flagdir <- if (lo_overlap > 0) "positive" else if (hi_overlap < 0) "negative" else "none"
  } else {
    all_bounds <- c(dfr$z1_ATE_LCI, dfr$z1_ATE_UCI, dfr$z2_ATE_LCI, dfr$z2_ATE_UCI)
    flagdir <- if (max(all_bounds, na.rm = TRUE) < 0) "negative" else if (min(all_bounds, na.rm = TRUE) > 0) "positive" else "none"
  }

  # ── Sample size caveat ─────────────────────────────────────────────────────
  sample_caveat <- if (!is.null(n_obs) && !is.na(n_obs) && n_obs < 500) {
    "Note: this dataset is relatively small, which means there may not have been enough data to detect a small-to-moderate effect even where the analyses agree. Treat directional conclusions with caution."
  } else {
    "These estimates are only as reliable as the variables included in your causal graph. If an important variable is missing, the true effect may differ."
  }

  # ══════════════════════════════════════════════════════════════════════════
  # NULL MODE — causal effect only, no model comparison
  # ══════════════════════════════════════════════════════════════════════════
  if (isTRUE(model_yn == "null")) {

    if (!overlap_exists) {
      return(paste0(
        "<strong>The two causal analyses gave conflicting answers.</strong><br><br>",
        "AIR estimates the effect of <em>", x_var, "</em> on <em>", y_var,
        "</em> using two different sets of background variables. ",
        "Here those two approaches disagree completely \u2014 ",
        "Analysis A estimates the effect at ", fmt_range(dfr$z1_ATE_LCI[1], dfr$z1_ATE_UCI[1]),
        ", while Analysis B estimates it at ", fmt_range(dfr$z2_ATE_LCI[1], dfr$z2_ATE_UCI[1]), ". ",
        "When the ranges don\u2019t even overlap, no single reliable conclusion can be drawn.<br><br>",
        "This often happens when the dataset is small, the causal graph includes uncertain relationships, ",
        "or a key variable is missing.<br><br>",
        "<strong>What to do:</strong> Review your causal graph for missing variables or incorrect edges. ",
        "Consider collecting more data before drawing conclusions."
      ))
    }

    direction_phrase <- if (flagdir == "positive") {
      paste0("is associated with a <strong>", fmt_range_abs(lo_overlap, hi_overlap),
             " increase</strong> in the probability of <em>", y_var, "</em>")
    } else if (flagdir == "negative") {
      paste0("is associated with a <strong>", fmt_range_abs(lo_overlap, hi_overlap),
             " decrease</strong> in the probability of <em>", y_var, "</em>")
    } else {
      paste0("does <strong>not meaningfully change</strong> the probability of <em>", y_var,
             "</em> \u2014 the effect is too small to distinguish from random variation")
    }

    consensus_text <- if (flagdir != "none") {
      paste0(
        "The range where both analyses agree \u2014 the most defensible single estimate \u2014 is ",
        "<strong>", fmt_range(lo_overlap, hi_overlap), "</strong>. "
      )
    } else {
      paste0(
        "The range where both analyses agree spans roughly ",
        "<strong>", fmt_range(lo_overlap, hi_overlap), "</strong>, ",
        "which AIR treats as no meaningful effect. "
      )
    }

    return(paste0(
      "<strong>Causal Effect Summary</strong><br><br>",
      "Based on your data and causal graph, changing <em>", x_var,
      "</em> from its baseline range to its experimental range ", direction_phrase, ".<br><br>",
      consensus_text, proximity_note, "<br><br>",
      ci_width_note, "<br><br>",
      "<strong>Caveat:</strong> ", sample_caveat
    ))
  }

  # ══════════════════════════════════════════════════════════════════════════
  # MODEL COMPARISON MODES: Yes / ATE / No
  # ══════════════════════════════════════════════════════════════════════════

  # ── State 1: No consensus ─────────────────────────────────────────────────
  if (!overlap_exists) {
    return(paste0(
      "<strong>The two causal analyses gave conflicting answers.</strong><br><br>",
      "AIR computes the effect of <em>", x_var, "</em> on <em>", y_var,
      "</em> using two different sets of background variables. ",
      "Here those two approaches disagree completely \u2014 ",
      "Analysis A estimates the effect at ", fmt_range(dfr$z1_ATE_LCI[1], dfr$z1_ATE_UCI[1]),
      ", while Analysis B estimates it at ", fmt_range(dfr$z2_ATE_LCI[1], dfr$z2_ATE_UCI[1]), ". ",
      "When the ranges don\u2019t even overlap, no single reliable conclusion can be drawn.<br><br>",
      "Wide uncertainty ranges often accompany this result and can indicate a small dataset, ",
      "uncertain relationships in the causal graph, or a missing variable driving the disagreement.<br><br>",
      "<strong>What to do:</strong> Review your causal graph for missing variables or incorrect edges. ",
      "Consider collecting more data before drawing conclusions."
    ))
  }

  # ── State 2: Internally inconsistent ──────────────────────────────────────
  if ((all(dfr$z1_ATE_UCI < dfr$z2_ATE) & all(dfr$z2_ATE_LCI > dfr$z1_ATE)) |
    ...
    dir_word <- if (flagdir == "positive") "a positive" else if (flagdir == "negative") "a negative" else "a possible"
    return(paste0(
      "<strong>The two analyses agree on direction but disagree on size.</strong><br><br>",
      "Both analyses find ", dir_word, " effect of <em>", x_var, "</em> on <em>", y_var,
      "</em>, but they differ meaningfully on how large that effect is. ",
      "Analysis A estimates <strong>", fmt_pct_signed(z1_pt),
      "</strong> and Analysis B estimates <strong>", fmt_pct_signed(z2_pt),
      "</strong> \u2014 a ", round(pt_gap * 100, 0), " percentage-point spread. ",
      "Their confidence ranges do overlap (roughly ", fmt_range(lo_overlap, hi_overlap),
      "), so a causal effect in that region is plausible, but the disagreement means ",
      "estimates should be treated as rough approximations rather than precise values.<br><br>",
      "This usually reflects limited data or structural uncertainty in the causal graph, ",
      "not necessarily a problem with your model."
    ))
  }

  # ── State 3: Standard interpretation ──────────────────────────────────────
  flag <- dfr$flag[1]

  inZ1 <- any(flag >= dfr$z1_ATE_LCI & flag <= dfr$z1_ATE_UCI, na.rm = TRUE)
  inZ2 <- any(flag >= dfr$z2_ATE_LCI & flag <= dfr$z2_ATE_UCI, na.rm = TRUE)
  bias_status <- if (inZ1 & inZ2) "unbiased" else if (inZ1 | inZ2) "partial" else "biased"

  # Degree of divergence from nearest CI boundary
  divergence <- if (!inZ1 & !inZ2) {
    if (flag > max(dfr$z1_ATE_UCI[1], dfr$z2_ATE_UCI[1])) {
      flag - max(dfr$z1_ATE_UCI[1], dfr$z2_ATE_UCI[1])
    } else {
      min(dfr$z1_ATE_LCI[1], dfr$z2_ATE_LCI[1]) - flag
    }
  } else if (!inZ1) {
    if (flag > dfr$z1_ATE_UCI[1]) flag - dfr$z1_ATE_UCI[1] else dfr$z1_ATE_LCI[1] - flag
  } else if (!inZ2) {
    if (flag > dfr$z2_ATE_UCI[1]) flag - dfr$z2_ATE_UCI[1] else dfr$z2_ATE_LCI[1] - flag
  } else {
    0
  }
  divergence_pp <- round(abs(divergence) * 100, 0)

  model_dir <- if (flag > hi_overlap) "overestimating" else if (flag < lo_overlap) "underestimating" else "accurate"

  # Bias source explanation with structural roles
  Z1_vars <- Zvars[Zvars$grp == "Z1", ]$Z
  Z2_vars <- Zvars[Zvars$grp == "Z2", ]$Z

  build_source <- function(vars, role) {
    if (length(vars) == 0 || all(is.na(vars)) || all(!nzchar(vars))) return(NULL)
    paste0("<em>", paste(vars, collapse = ", "), "</em> (", role, ")")
  }

  z1_role <- paste0(
    "direct background ", if (length(Z1_vars) == 1) "cause" else "causes",
    " of <em>", x_var, "</em>"
  )
  z2_role <- paste0(
    if (length(Z2_vars) == 1) "a variable" else "variables",
    " that influence the pathway between <em>", x_var, "</em> and <em>", y_var, "</em>"
  )

  bias_sources <- Filter(Negate(is.null), list(
    if (!inZ1) build_source(Z1_vars, z1_role) else NULL,
    if (!inZ2) build_source(Z2_vars, z2_role) else NULL
  ))

  bias_source_text <- if (bias_status != "unbiased" && length(bias_sources) > 0) {
    paste0(
      "<strong>Likely source of bias:</strong> ",
      paste(bias_sources, collapse = "; and "),
      " \u2014 highlighted in red on the causal graph. ",
      "Review these variables and the edges connected to them."
    )
  } else ""

  # Classifier spread (No mode only)
  classifier_text <- ""
  if (isTRUE(model_yn == "No")) {
    clf <- classifier_buffer()
    if (!is.null(clf) && nrow(clf) > 1) {
      clf_flags <- clf$flag[clf$algorithm != "ML Model Summary"]
      clf_flags <- clf_flags[!is.na(clf_flags)]
      if (length(clf_flags) > 1) {
        clf_spread <- max(clf_flags) - min(clf_flags)
        classifier_text <- if (clf_spread < 0.05) {
          paste0(
            "The five classifiers AIR trained internally gave consistent predictions ",
            "(range: ", fmt_range_abs(min(clf_flags), max(clf_flags)), "), ",
            "which reinforces this conclusion."
          )
        } else if (clf_spread < 0.15) {
          paste0(
            "The five classifiers AIR trained internally were moderately consistent ",
            "(range: ", fmt_range_abs(min(clf_flags), max(clf_flags)), ")."
          )
        } else {
          paste0(
            "Note: the five classifiers AIR trained internally gave a wide spread of predictions ",
            "(range: ", fmt_range_abs(min(clf_flags), max(clf_flags)), "). ",
            "This spread suggests the model may be sensitive to the choice of algorithm. ",
            "Interpret the specific effect size with caution."
          )
        }
      }
    }
  ...
  # Headline
  headline <- if (bias_status == "unbiased") {
    "<strong>No bias detected.</strong>"
  } else if (bias_status == "partial") {
    "<strong>Possible bias detected.</strong>"
  } else {
    "<strong>Bias detected.</strong>"
  }

  # What the causal analysis found
  direction_phrase <- if (flagdir == "positive") {
    paste0("a <strong>", fmt_range_abs(lo_overlap, hi_overlap),
           " increase</strong> in the probability of <em>", y_var, "</em>")
  } else if (flagdir == "negative") {
    paste0("a <strong>", fmt_range_abs(lo_overlap, hi_overlap),
           " decrease</strong> in the probability of <em>", y_var, "</em>")
  } else {
    paste0("<strong>no meaningful change</strong> in the probability of <em>", y_var,
           "</em> \u2014 the effect is too small to distinguish from random variation")
  }

  found_text <- paste0(
    "<strong>What the causal analysis found:</strong> When <em>", x_var,
    "</em> moves from its baseline range to its experimental range, the expected outcome is ",
    direction_phrase, ". ", proximity_note
  )

  # Model comparison paragraph
  model_pct      <- fmt_pct_signed(flag)
  consensus_range <- fmt_range(lo_overlap, hi_overlap)

  model_para <- if (bias_status == "unbiased") {
    paste0(
      "Your model\u2019s predicted effect (", model_pct,
      ") falls within the range both analyses expect (", consensus_range,
      "). The model is performing as expected."
    )
  } else if (flagdir == "none") {
    paste0(
      "Your model predicts an effect of ", model_pct, " on <em>", y_var,
      "</em>, but the causal analysis finds no real effect ",
      "(both analyses agree the effect is near zero: ", consensus_range, "). ",
      "The relationship your model learned is likely spurious."
    )
  } else {
    over_under_word  <- if (model_dir == "overestimating") "overstating" else "understating"
    divergence_clause <- if (divergence_pp > 0) {
      paste0(" \u2014 your model is ", over_under_word, " the effect by roughly <strong>",
             divergence_pp, " percentage points</strong>")
    } else ""
    warning_clause <- if (bias_status == "partial") {
      " Your model\u2019s prediction falls within one analysis range but outside the other, which is a warning sign rather than a definitive verdict."
    } else {
      " Your model\u2019s prediction falls outside both analysis ranges."
    }
    paste0(
      "Your model predicts an effect of ", model_pct, " on <em>", y_var,
      "</em>, but the causal analysis puts the best estimate at ", consensus_range,
      divergence_clause, ".", warning_clause
    )
  }
  ...
  parts <- c(
    ...
    "<br><br>",
    found_text,
    if (nzchar(bias_source_text)) c("<br><br>", bias_source_text) else NULL,
    if (nzchar(classifier_text))  c("<br><br>", classifier_text)  else NULL,
    "<br><br>",
    ci_width_note,
    if (bias_status != "biased") c("<br><br>", "<strong>Caveat:</strong> ", sample_caveat) else NULL
  )
...
get_histogram_x <- function(df, xvar, dir, threshold) {
  ...
  ggplot(plot_df, aes(x = x, fill = Treatment, colour = Treatment)) +
    ...
    theme(text             = element_text(colour = "#6c757d", face = "bold"),
...
get_updated_graph <- function(dot, xvar, yvar, Zvars = NULL) {
  ...
  make_decl <- function(node, colour, fontcolor = NULL) {
    attrs <- paste0('style=filled, fillcolor="', colour, '"')
    if (!is.null(fontcolor)) attrs <- paste0(attrs, ', fontcolor="', fontcolor, '"')
    sprintf('  "%s" [%s];', node, attrs)
  }
  ...
  new_lines <- c(
    ...
    if (nzchar(yvar)) sapply(yvar, make_decl, colour = "#0D47A1", fontcolor = "white"),
    if (length(Z1))   sapply(Z1,   make_decl, colour = "#8A93A6"),
    if (length(Z2))   sapply(Z2,   make_decl, colour = "#D6DAE3")
  )
...
get_final_graph <- function(dot, xvar, yvar, Zvars, dfr = results_out_buffer()) {
  ...
  dot <- change_node_color(dot, yvar, "'#0D47A1'", "'white'")
...
if (in_z1 && in_z2) {
  ...
  # Inside one, outside the other → paint only the one that's outside;
  # the in-range set is NOT highlighted, so it gets grayed with everything else
  flagged <- character(0)
  if (!in_z1) {
    dot <- paint_nodes(dot, Z1, "'#DDB64B'")
    flagged <- c(flagged, Z1)
    log_debug("flag outside Z1 only")
  }
  if (!in_z2) {
    dot <- paint_nodes(dot, Z2, "'#DDB64B'")
    flagged <- c(flagged, Z2)
    log_debug("flag outside Z2 only")
  }
  # something's suspect -- mute everything but X/Y/the flagged set, sparing
  # the pathways that carry the flagged bias into X/Y
  paths <- dot_path_sets(dot, flagged, c(xvar, yvar))
  dot <- gray_out_background(dot, c(xvar, yvar, flagged),
                             path_fwd = paths$fwd, path_bwd = paths$bwd)

} else if (!in_z1 && !in_z2) {
  # Outside both → paint both in red; white text keeps labels legible
  # against the dark fill
  dot <- paint_nodes(dot, Z1, "'#B6202D'", "'white'")
  dot <- paint_nodes(dot, Z2, "'#B6202D'", "'white'")
  # biased -- bold only the red-node edges that lie on a Z → X/Y pathway,
  # then mute everything but X/Y/Z1/Z2 and those pathways
  paths <- dot_path_sets(dot, c(Z1, Z2), c(xvar, yvar))
  dot <- bold_edges_from(dot, c(Z1, Z2), only_into = paths$bwd)
  dot <- gray_out_background(dot, c(xvar, yvar, Z1, Z2),
                             path_fwd = paths$fwd, path_bwd = paths$bwd)
...
detect_time_columns <- function(data, parse_threshold = 0.8, sample_n = 100) {

  name_pattern <- paste0(
    "(?i)(^|_|\\.|\\s)",
    "(date|time|timestamp|datetime|year|month|day|hour|minute|second|",
    "period|qtr|quarter|week|epoch|created|updated|modified|",
    "start|end|begin|finish|ts|dt)",
    "(_|\\.|\\s|$)"
  )

  per_col <- lapply(names(data), function(col) {
    x <- data[[col]]

    class_hit <- inherits(x, c("Date", "POSIXct", "POSIXlt", "POSIXt",
                               "yearmon", "yearqtr", "hms", "difftime"))

    name_hit <- grepl(name_pattern, col, perl = TRUE) ||
                grepl("^(t|ts|dt)$", col, ignore.case = TRUE)

    parse_hit <- FALSE
    parse_rate <- 0
    if (is.character(x) || is.factor(x)) {
      vals <- na.omit(as.character(x))
      ...
      if (length(vals) > 0) {
        ...
        parse_hit  <- parse_rate >= parse_threshold
      }
    }

    numeric_time_hit <- FALSE
    if (is.numeric(x)) {
      vals <- na.omit(x)
      if (length(vals) > 0) {
        is_yearlike <- all(vals >= 1800 & vals <= 2100) && all(vals == floor(vals))
        is_unix_s   <- all(vals >= 6.3e8  & vals <= 2.5e9)
        is_unix_ms  <- all(vals >= 6.3e11 & vals <= 2.5e12)
        numeric_time_hit <- is_yearlike || is_unix_s || is_unix_ms
      }
    }

    is_time <- class_hit || name_hit || parse_hit || numeric_time_hit

    list(
      column  = col,
      is_time = is_time,
      reasons = c(
        if (class_hit)        "temporal class",
        if (name_hit)         "name pattern",
        if (parse_hit)        sprintf("parsed %.0f%% as datetime", 100 * parse_rate),
        if (numeric_time_hit) "numeric time encoding"
      )
    )
  })

  names(per_col) <- names(data)
  time_cols <- names(data)[vapply(per_col, `[[`, logical(1), "is_time")]

  list(
    is_time_series = length(time_cols) > 0,
...
TetradSearch <- setRefClass(
  ...
  methods = list(
    ...
    run_boss = function(num_starts = 30, use_bes = FALSE, time_lag = 0, use_data_order = TRUE, output_cpdag = TRUE, is_time_series = FALSE) {
      ...
      .self$.setParam("timeLagReplicatingGraph", is_time_series)
      .self$.setParam("useDataOrder", use_data_order)
      .self$.setParam("outputCpdag", output_cpdag)
      ### add time lag and time lage replicating graph here
    ...
    markov_check = function(graph, percent_resample = 1, condition_set_type = "ORDERED_LOCAL_MARKOV_PROPERTY",
      ...
      test_ <- safe_jcall(.self$mc_test, "Ledu/cmu/tetrad/search/test/IndependenceTest;",
                      "getTest", dataModel, .self$params)
      
      mc <- .jnew("edu.cmu.tetrad.search.MarkovCheck", graph, test_, condition_set_type_)
      
      # Configure it
      safe_jcall(mc, "V", "setFractionResample", as.double(percent_resample))
      ...
      bin_indep <- safe_jcall(mc, "D", "getBinomialPValue_", TRUE)
      bin_dep <- safe_jcall(mc, "D", "getBinomialPValue_", FALSE)
...
uiOutput("ui_file1")
div(class = "text-end", uiOutput("ui_ts_select"))  # Bootstrap 5
uiOutput('ui_file2')
...
-   Graph Colors
    ...
    -   [Navy]{style="color: #0D47A1; font-weight: bold;"}: Y Variable of interest, selected by the user in step 2.
    -   [Dark Gray]{style="color: #8A93A6; font-weight: bold;"}: Any nodes with this color have been identified as belonging to the first identified adjustment set, used to calculate causal effect estimates.
    -   [Light Gray]{style="color: #D6DAE3; font-weight: bold;"}: Any nodes with this color have been identified as belonging to the second identified adjustment set, used to calculate causal effect estimates.
    -   [Red]{style="color: #B6202D; font-weight: bold;"}: Any nodes with this color have been flagged as introducing bias into the results of the input model. Nodes will change from (dark/light) gray to red if the model ATE falls outside the 95% confidence interval for a given adjustment set.
-   Histogram Colors
...
-   Ribbon Plot Colors
    -   [Red]{style="color: #B6202D; font-weight: bold;"}: The range of effect sizes that are outside the 95% confidence intervals of both adjustment sets. Values falling in this range are considered non-significant.
    -   [Yellow]{style="color: #DDB64B; font-weight: bold;"}: The range of effect sizes that are outside of one 95% confidence interval for one effect size, but inside for the other. Values falling in this range are considered suspect, and should be closely monitored for signs of bias.
    -   [Green]{style="color: #2E8F5C; font-weight: bold;"}: The range of effect sizes that are inside both adjustment set's 95% confidence intervals. Values falling in this range are consistent with those calculated by AIR's causal estimation and are considered bias-free.
    -   [Dark Gray]{style="color: #8A93A6; font-weight: bold;"}: The dot (causal estimate) and lines (95% confidence interval) drawn in this color correspond to the first identified adjustment set in your data. 
    -   [Light Gray]{style="color: #D6DAE3; font-weight: bold;"}: The dot (causal estimate) and lines (95% confidence interval) drawn in this color correspond to the second identified adjustment set in your data. 
 
# Logs {layout-ncol=2}
:::: {.columns}
::: {.column width=100%}
```{r running-log, echo=FALSE}
uiOutput("live_log_tail", 
...
output$ui_reset_btn <- renderUI({
  req(graph_complete())
  actionButton(
    "reload_btn", "↻ Reset Selections",
    class = "btn btn-light btn-sm",
    style = "width:100%; color:#212529;"
  )
})
# Live log tail tracker ----
output$live_log_tail <- renderUI({
  logfile <- tryCatch(
    get(".airtool_log_file", envir = .GlobalEnv),
  ...
  if (is.null(logfile) || !file.exists(logfile)) {
    return(pre("No active log file found."))
  }

  log_tail <- tryCatch(
    tail(readLines(logfile, warn = FALSE), n = 25),
    error = function(e) "<unable to read log file>"
  )

  msg <- paste(log_tail, collapse = "\n")
  msg <- gsub("ERROR", "<b style='color:red'>ERROR</b>", msg, fixed = TRUE)
  msg <- gsub("WARN",  "<b style='color:orange'>WARN</b>", msg, fixed = TRUE)
  msg <- gsub("INFO",  "<b style='color:green'>INFO</b>", msg, fixed = TRUE)

  HTML(paste0(
    "<div style='height:100%; min-height:100%; display:flex; flex-direction:column;'>",
      "<div style='flex:1; height:100%; overflow-y:auto; ",
                  "background-color:#f9f9f9; color:#212529; ",
                  "font-family:monospace; font-size:0.9em; padding:10px; border-radius:6px;'>",
        "<strong>Live log:</strong><br><br>",
        "<pre style='white-space:pre-wrap; margin:0; height:100%;'>",
           msg,
        "</pre>",
      "</div>",
    "</div>"
...
output$file1_txt <- renderUI({
  req(input$file1)
  res <- detect_time_columns(df())
  is_ts <- res$is_time_series # boolean for whether time series was detected
  res$time_colums # actual variables that were flagged as time series
  if (is_ts){
    tscols <- length(res$time_columns)
    HTML(paste0("<i>Found ", nrow(df()), " instances of ", ncol(df()), " variables and ", tscols, " possible time series variable(s)</i>"))
  } else{
...
df <- reactive({
  ...
  shiny::validate(
    shiny::need(ext %in% c("csv", "xlsx"), "Data file must be .csv or .xlsx"),
    shiny::need(file.size(input$file1$datapath) < 5000 * 1024^2, "File too large (500 MB limit)")
  )
  ...
  tryCatch({
    if (ext == "csv") {
      ...
      # ← IMPORTANT: don’t force .default = col_number()
      df <- readr::read_delim(
        input$file1$datapath,
        delim = delim,
        guess_max = 10000,         # improve type guessing on wide/long files
    ...
    # If you truly need to coerce only specific columns, do it TARGETED, not blanket:
    # df[c(1, ncol(df))] <- lapply(df[c(1, ncol(df))], readr::parse_number)
...
high_corr_pairs <- reactive({
  ...
  # drop constant / all-NA columns (cor() would return NA)
  keep <- vapply(num, function(x) {
    x2 <- stats::na.omit(x)
    length(unique(x2)) > 1
  }, logical(1))
  ...
  # compute correlation
  cm <- stats::cor(num, use = "pairwise.complete.obs", method = "pearson")
  # keep only upper triangle (no dupes, no diagonal)
  cm[lower.tri(cm, diag = TRUE)] <- NA

  thr <- 0.9  # make this an input if you want
...
observe({
  ...
  # Nice HTML list for readability inside SweetAlert
  items <- paste0(
    "<ul>",
    paste(sprintf("<li><b>%s</b> &amp; <b>%s</b> (r = %.2f)</li>", pairs$var1, pairs$var2, pairs$r),
          collapse = ""),
    "</ul>"
  )
...
if (nrow(pairs) > 0) {
  ...
  items <- paste0(
    ...
    paste(
      sprintf(
        "<li><span style='display:inline-block; width:4.5em; text-align:right;'><b>%s</b></span>  [%s -- %s]</li>",
        fmt_num(pairs$r),
        esc(pairs$var1),
        esc(pairs$var2)
      ),
  ...
  shinyWidgets::sendSweetAlert(
    ...
    text    = htmltools::HTML(paste0(
      "<p><b>Some variables in your dataset are strongly correlated (|r| ≥ 0.90).</b></p>",
      "<p>This may indicate redundant information and can affect downstream analyses such as regression or causal estimation.</p>",
      "<hr style='margin:8px 0;'>",
      "<p><b>Highly correlated pairs:</b></p>",
      items
    )),
    type = "warning",
    btn_labels = "OK",
...
r_ts_select <- reactive({
  ts_val <- input$ts_select
  return(ts_val)
})
...
## read raw inputs directly (not via vars_sel()) so that picking Y doesn't
## spuriously invalidate x_ready() (and vice versa) -- vars_sel() bundles
## xvar+yvar into one reactive, so changing either one invalidates both
x_ready   <- reactive({ !is.null(input$xvar) && nzchar(input$xvar) })  # X chosen & valid
y_ready   <- reactive({ !is.null(input$yvar) && nzchar(input$yvar) })  # Y chosen & valid
...
## scoped single-variable versions of vars_sel()$x / vars_sel()$y -- use these
## (not vars_sel() directly) in anything that should only react to ONE of
## X/Y, e.g. per-variable renderUI/renderPlot outputs. Reading vars_sel()$x
## still subscribes to input$yvar too, since they're bundled into one
## reactive -- that's what caused X's threshold widget (and others) to
## needlessly rebuild every time Y was picked, and vice versa
x_val <- reactive({
  req(df())
  if (!is.null(input$xvar) && nzchar(input$xvar)) check_column_exists(df(), input$xvar, "xvar") else ""
})
y_val <- reactive({
  req(df())
  if (!is.null(input$yvar) && nzchar(input$yvar)) check_column_exists(df(), input$yvar, "yvar") else ""
})
...
highlighted_graph <- reactive({
  ...
  if (!is.null(vars_sel()$y) && nzchar(vars_sel()$y))
    dot <- change_node_color(dot, vars_sel()$y, "'#0D47A1'", "'white'")


  if (!is.null(Z) && nrow(Z)) {
      if (any(Z$grp == "Z1")) dot <- change_node_color(dot, Z[Z$grp == "Z1", "Z"], "'#8A93A6'")
      if (any(Z$grp == "Z2")) dot <- change_node_color(dot, Z[Z$grp == "Z2", "Z"], "'#D6DAE3'")
  }  
...
## ── phantom-click guard for renderUI-created action buttons ──────────────
## When a dynamically rendered actionButton's DOM is rebuilt/rebound (the
## sidebar re-renders widgets after variable selection), its client-side click
## counter resets to 0. Shiny reports that 0 as a "change", firing observeEvent
## with no user click -- a phantom Reset here used to wipe the X/Y dropdowns.
## Only counter INCREASES are real clicks.
make_click_guard <- function(id) {
  last <- reactiveVal(0)
  function(val) {
    new <- suppressWarnings(as.numeric(val %||% 0))
    if (!isTRUE(is.finite(new))) new <- 0
    old <- last()
    last(new)
    real <- isTRUE(new > old)
    if (!real) log_debug("Ignored phantom {id} event (counter {old} -> {new})",
                         id = id, old = old, new = new)
    real
  }
}
reload_click_real <- make_click_guard("reload_btn")
build_click_real  <- make_click_guard("buildButton")
go_click_real     <- make_click_guard("goButton")

observeEvent(input$reload_btn, {
  ## ── "Reset Selections" – soft-reset back to Step 2 ─────────────────────
  ## Preserves: uploaded files, parsed data, knowledge, causal graph
  ## Resets:    variable selections, thresholds, model config, all results
    req(reload_click_real(input$reload_btn))
    req(graph_complete())        # only meaningful if a graph exists

    # ── 1. Clear all result / estimation buffers ──────────────────────────

    results_buffer(data.frame())
    results_out_buffer(data.frame())
    datafile_buffer(NULL)
    superlearner_output_buffer(list())
    tmle_buffer(data.frame())
    ml_buffer(data.frame())
    combined_buffer(data.frame())
    classifier_buffer(NULL)

    # ── 2. Reset control flags ────────────────────────────────────────────
    locked(FALSE)
    calc_complete(FALSE)
    graph_updated(FALSE)
    # NOTE: step1lock and graph_complete intentionally left alone

    # ── 3. Clear per-run reactive values (keep graph artefacts) ───────────
    rv$Zvars       <- NULL
    rv$model_yn    <- NULL
    rv$model_ate   <- NULL
    rv$data_caveat <- NULL

    # ── 4. Tidy up global-env leftovers from the previous run ─────────────
    for (nm in c("Zvars", "df_vars", "results_detailed")) {
      if (exists(nm, envir = .GlobalEnv)) rm(list = nm, envir = .GlobalEnv)
    }

    # ── 5. Reset UI inputs back to blanks ─────────────────────────────────
    updateSelectInput(session, "xvar", selected = "")
    updateSelectInput(session, "yvar", selected = "")
    # Threshold / direction inputs will vanish automatically because their

    # renderUI calls req(vars_sel()$x) / req(vars_sel()$y), which fail
    # when the selects are empty.

    # ── 6. Re-render the plain (uncoloured) causal graph ──────────────────
    output$blankGraph <- renderGrViz({
      req(graph_complete(), rv$dot)
      dot <- sub(
        "(digraph[^\\{]*\\{)",
        "\\1\n  graph [tooltip=\"Causal graph\"]",
        rv$dot, perl = TRUE
      )
      grViz(dot)
    })

    log_info("Reset Selections: state rolled back to Step 2.")

    sendSweetAlert(
      session = shiny::getDefaultReactiveDomain(),
      title   = "Selections Reset",
      text    = "Your causal graph has been preserved. Please select new X and Y variables to continue.",
      type    = "info",
      btn_labels        = "OK",
...
observeEvent(input$buildButton, {
  req(build_click_real(input$buildButton))
  ...
  if (identical(adj_list, NULL)){
    ...
    if (adj_list[2] == "edges"){
      sendSweetAlert(
        ...
        text  = "Undirected edges detected in graph. Adjustment sets may or may not be missing
                due to undirected edges. If undirected edges exist near your variables of interest,
                consider revising knowledge file so that search result has no undirected edges
                and try again. Ensure that nodes on either side of an undirected edge are on
                separate levels in the knowledge file to ensure directionality is enforced.
                If this does not resolve the issue, there may simply be no valid adjustment
                sets (in this case, the undirected edges are likely a red herring).
                Automatic resolution of this issue is not yet supported by the AIR
                Tool, so users must resolve this issue manually.",
        type  = "error",
        closeOnClickOutside = FALSE
      )
    } else if (adj_list[2] == "structure"){
      sendSweetAlert(
        session = shiny::getDefaultReactiveDomain(),
        title = "No adjustment set found",
        text  = "The structure of this data is not amenable to causal inference as no adjustment sets can be found.",
        type  = "error",
        closeOnClickOutside = FALSE
      )
    }
...
observeEvent(input$goButton, {
  req(go_click_real(input$goButton))
  req(both_ready())
  req(graph_complete())
  ...
  shiny::need(v$y %in% get_X_descendents(v$x, graphtxt_buffer),
    ...
    withProgress(message = 'Building Causal Graph', style = "notification", value = 0.1, {
      ...
      tryCatch({
        ...
        # Time-series mode: materialize col:n columns so workers can use them as
        # TMLE confounders. Tetrad names lagged variables as `col:n` (e.g.
        # enginehours_max:1). Lags are computed in original data order, matching
        # BOSS (use_data_order = TRUE).
        ts_lag <- if (isTRUE(r_ts_select())) 1L else 0L
        if (ts_lag > 0L) {
          lag_cols <- names(df_snapshot)[
            vapply(df_snapshot, function(x) {
              is.numeric(x) || is.logical(x) ||
                inherits(x, c("Date", "POSIXct", "POSIXlt", "POSIXt"))
            }, logical(1L))
          ]
          for (col in lag_cols) {
            df_snapshot[[paste0(col, ":", ts_lag)]] <- dplyr::lag(df_snapshot[[col]], n = ts_lag)
          }
          # The first ts_lag rows have no prior values; drop them so sl3/tmle3
          # never sees NA covariates.
          lagged_names <- paste0(lag_cols, ":", ts_lag)
          df_snapshot  <- df_snapshot[complete.cases(df_snapshot[, lagged_names, drop = FALSE]), ]
          log_debug("Time-series lag={lag}: created {n} lagged columns; {r} rows remain after NA drop.",
                    lag = ts_lag, n = length(lag_cols), r = nrow(df_snapshot))
        }

        # ── preflight: enough data to estimate? ──────────────────────────
        # 20-fold stratified CV needs every fold to contain both classes of
        # the binarized treatment and outcome. Too few rows (especially
        # after time-series lagging) or a lopsided threshold split makes
        # every learner fail and the whole run collapse downstream.
        n_folds <- 20
        xv <- df_snapshot[[vars_sel()$x]]
        yv <- df_snapshot[[vars_sel()$y]]
        treated <- if (isTRUE(tv_dir_val)) xv >= tv_threshold_val else xv <= tv_threshold_val
        success <- if (isTRUE(ov_dir_val)) yv >= ov_threshold_val else yv <= ov_threshold_val
        group_n <- c(sum(treated,  na.rm = TRUE), sum(!treated, na.rm = TRUE),
                     sum(success,  na.rm = TRUE), sum(!success, na.rm = TRUE))
        names(group_n) <- c(
          paste0("experimental (", vars_sel()$x, " ", if (isTRUE(tv_dir_val)) "≥" else "≤", " ", tv_threshold_val, ")"),
          "baseline",
          paste0("success (", vars_sel()$y, " ", if (isTRUE(ov_dir_val)) "≥" else "≤", " ", ov_threshold_val, ")"),
          "fail"
        )
        min_class <- min(group_n)
        group_txt <- paste0(names(group_n), " = ", group_n, collapse = ", ")

        # tier 1 -- hopeless: a group with <2 rows (nothing to learn from) or
        # fewer rows than CV folds. Abort before any learners run.
        if (min_class < 2L || nrow(df_snapshot) < n_folds) {
          estimation_aborted <- TRUE
          locked(FALSE)   # let the user adjust selections and retry immediately
          sendSweetAlert(
            session = shiny::getDefaultReactiveDomain(),
            title = "Not Enough Data to Estimate",
            text = paste0(
              nrow(df_snapshot), " rows are available for estimation",
              if (ts_lag > 0L) " after time-series lagging" else "",
              ". Group sizes: ", group_txt,
              ". A group with fewer than 2 rows cannot support estimation at all. ",
              "Consider a larger dataset",
              if (ts_lag > 0L) ", disabling time-series mode," else "",
              " or thresholds that split the data less unevenly."
            ),
            type = "warning",
            btn_labels = "OK",
            btn_colors = "#1976d2",
          ...
          stop("Insufficient data for estimation (rows=", nrow(df_snapshot),
               ", ", group_txt, ", folds=", n_folds, ")")
        }

        # tier 2 -- thin but not hopeless: run anyway, warn now, and carry a
        # caveat into the results so the output itself says to be careful
        rv$data_caveat <- NULL
        if (min_class < n_folds || nrow(df_snapshot) < 2L * n_folds) {
          rv$data_caveat <- paste0(
            "Limited data: only ", nrow(df_snapshot),
            " rows were available for estimation",
            if (ts_lag > 0L) " after time-series lagging" else "",
            " (group sizes: ", group_txt, "). With any group below ", n_folds,
            " rows, the ", n_folds, "-fold cross-validated estimates below may ",
            "be unstable — interpret them with caution."
          )
          log_info("Data caveat attached to this run: {msg}", msg = rv$data_caveat)
          sendSweetAlert(
            session = shiny::getDefaultReactiveDomain(),
            title = "Limited Data — Proceeding with Caution",
            text = paste0(rv$data_caveat, " The analysis will continue."),
            type = "warning",
            btn_labels = "Continue",
            btn_colors = "#1976d2",
            closeOnClickOutside = TRUE
        ...
        tryCatch({
          ...
          sendSweetAlert(
            ...
            text = paste0(
              "The causal estimates could not be computed (",
              conditionMessage(e), "). ",
              "This usually means every machine-learning learner failed to ",
              "train — most often because one treatment/outcome group is too ",
              "small or imbalanced to support cross-validation. Consider a ",
              "larger dataset or thresholds that split the data less ",
              "unevenly. See the log for details."
            ),
            type = "error",
            btn_labels = "OK",
            btn_colors = "#1976d2",
        ...
        # preflight abort already showed its own (more specific) alert
        if (!estimation_aborted) {
          sendSweetAlert(
            session = shiny::getDefaultReactiveDomain(),
            title = "Estimation Failed",
            text = paste0("The causal estimation could not be completed. ",
...
output$second_column_content <- renderUI({
    if (calc_complete()) {
      tags$div(
        ...
        tags$div(
          style = "flex: 0 0 auto;",   # shrink to content -- no reserved dead space
...
output$xvar <- renderUI({
  ...
  tagList(
    ...
    tags$style(HTML("#xvar option:disabled { color:#6c757d; }")),
    tags$script(HTML(sprintf("
      (function(){
        $(document).off('.xvarBlock');
        if (window.__xvarBlockInterval) { clearInterval(window.__xvarBlockInterval); }

        var blocked = %s;

        function applyXDisabled(){
          ...
          // deliberately no force-reset of sel.value here: server-side validate()
          // rejects blocked picks, and a JS reset can silently blank a user's pick
        ...
        // NOTE: deliberately NOT bound to document-wide shiny:value/shiny:visualchange --
        // those fire on EVERY Shiny output update anywhere on the page (e.g. the live
        // log tail re-renders every second), which was toggling these <option> elements
        // while the native dropdown was open and silently swallowing the user's click.
        // A fresh render (which re-runs this whole script) already fires whenever the
        // choices can actually change, so the short poll below is enough.
        var tries=0;
        window.__xvarBlockInterval = setInterval(function(){
          applyXDisabled();
          if(++tries>20) clearInterval(window.__xvarBlockInterval);
        }, 100);
      })();
    ", jsonlite::toJSON(blocked))))
  )
})
...
output$yvar <- renderUI({
  ...
  if (isTRUE(locked())) {
    ...
    tagList(
      ...
      tags$style(HTML("#yvar option:disabled { color:#6c757d; }")),
      tags$script(HTML(sprintf("
        (function(){
          // Remove previous handler to prevent accumulation
          $(document).off('.yvarBlock');
          if (window.__yvarBlockInterval) { clearInterval(window.__yvarBlockInterval); }

          var blocked = %s;

          function applyYDisabled(){
            var sel = document.getElementById('yvar');
            if(!sel) return;

            // Re-enable ALL first, then disable current blocked set
          ...
          // choices can actually change, so the short poll below is enough.
          var tries=0;
          window.__yvarBlockInterval = setInterval(function(){
            applyYDisabled();
            if(++tries>20) clearInterval(window.__yvarBlockInterval);
          }, 100);
        })();
      ", jsonlite::toJSON(blocked_y))))
    )
  }
})
...
# ── Reset Y and downstream state when X changes ──────────────────
observeEvent(input$xvar, ignoreInit = TRUE, {
  # Only act if graph exists and X is valid
  req(graph_complete(), nzchar(input$xvar))

  # Reset Y selection to force a fresh pick
  updateSelectInput(session, "yvar", selected = "")
  
  # Clear stale adjustment set state
  rv$Zvars <- NULL
  graph_updated(FALSE)
})
...
output$ui_threshold_x <- renderUI({
  ...
  dir_val <- {
          prev <- isolate(input$tv_dir_in)
          if (!is.null(prev) && !is.na(prev)) prev else TRUE
          }
  ...
  tags$span(paste("Test the effect when ", x_val(), " is ")),
...
output$ui_threshold_y <- renderUI({
  ...
  tags$div(
  style = "display:flex;align-items:center;gap:.5rem;flex-wrap:wrap;",
  tags$span(paste("Where success is defined as ", y_val(), " being ")),
...
output$ui_file1 <- renderUI({
  ...
  div(
    ...
    tags$div(
      style = "margin-bottom: 10px; color: #6c757d; font-style: italic;",
...
output$ui_ts_select <- renderUI({
  checkboxInput("ts_select", label = "Time Series?", value = FALSE)
})
...
output$histogram_x <- renderPlot({
  req(x_val(), !is.null(input$tv_threshold))   # wait for slider
  p <- get_histogram_x(df(), x_val(), tv_dir(), tv_val())
  p
  }, bg = "transparent")

output$histogram_y <- renderPlot({
  req(y_val(), !is.null(input$ov_threshold))   # wait for slider
  p <- get_histogram_y(df(), y_val(), ov_dir(), ov_val())
  p
  }, bg = "transparent")
...
output$ui_model_exist <- renderUI({
  req(graph_ready())
  req(graph_updated())
  selected_val <- if (locked()) isolate(input$model_exist) else "null"
  tagList(
    selectInput(
      inputId  = "model_exist",
      ...
      choices  = c(
        ...
        "No: Just show me the effect" = "null"
      ),
      selected = selected_val,
      width    = "100%"
    ),
...
output$ui_ate_upload = renderUI({
  ...
  if (input$model_exist == "ATE") {
    ...
    if (locked()) {
      ...
      tags$div(
        style = "color: #6c757d; font-style: italic; margin-top: 0.5em;",
...
output$ribbon_plot <- renderImage(
  {
    ...
    # wide canvas + hugging height: line widths are fixed in mm, so a wider
    # canvas renders proportionally thinner marks once CSS stretches the
    # image to the column width; a taller canvas would just bake white bands
    # above/below the aspect.ratio-pinned panel into the PNG
    ggsave(tmp, plot = plt,
           width = 10, height = 2.2, units = "in", bg = "transparent")
           list(src         = tmp,
...
output$figurecaption = renderUI({
  ...
  # bold the lead-in; done here (not in get_figure_caption) so the plain
  # string stays usable in the PDF report
  caption <- sub("^Risk Difference:", "<strong>Risk Difference:</strong>", caption)
  # font size matches the interpretation section; bottom margin gives the
  # caption breathing room before the divider
  tags$div(style = "font-size:24px; margin-bottom: 1em;", HTML(caption))
  })
...
output$ui_interpretation = renderUI({
  ...
  result_text <- get_ui_interpretation(AIRHome, df_vars, Zvars,
                                       model_yn = rv$model_yn,
                                       n_obs    = nrow(datafile_buffer()))

  tagList(
    tags$h3("Interpreting your results:"),
    if (!is.null(rv$data_caveat))
      tags$div(
        style = paste0("font-size:24px; border-left: 4px solid #DDB64B; ",
                       "padding-left: 12px; margin-bottom: 1em;"),
        tags$strong("Caution: "), rv$data_caveat
      ),
    tags$div(style = "font-size:24px;", HTML(result_text))
  )
})
  ...
  content = function(file) {
    ...
    tryCatch({
      ...
      result_text  <- get_ui_interpretation(AIRHome, df_vars, vals$Zvars,
                                            model_yn = isolate(rv$model_yn),
                                            n_obs    = nrow(vals$df))
      # carry the limited-data caveat into the PDF report (markdown -- the
      # report renders result_text inline)
      data_caveat <- isolate(rv$data_caveat)
      if (!is.null(data_caveat)) {
        result_text <- paste0("**Caution:** ", data_caveat, "\n\n", result_text)
      }
