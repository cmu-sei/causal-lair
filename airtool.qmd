...
bold_edges_from <- function(dot, nodes, penwidth = 2) {
  nodes <- nodes[nzchar(nodes)]
...
gray_out_background <- function(dot, highlighted, node_color = "#B9BEC7",
  ...
  others <- setdiff(all_dot_nodes(dot), c(highlighted, on_path))
  dot <- mute_nodes(dot, others, node_color, node_font)

  lines <- strsplit(dot, "\n", fixed = TRUE)[[1]]
  fade_edge <- function(line) {
    if (!is_edge_line(line)) return(line)
    if (first_node(line) %in% highlighted) return(line)  # black only *from* a highlighted node
...
if (in_z1 && in_z2) {
  ...
  dot <- bold_edges_from(dot, c(Z1, Z2))
  # biased -- mute everything but X/Y/Z1/Z2, sparing the pathways that carry
  # the flagged bias into X/Y
