...
log_file <- file.path(
  log_dir,
  sprintf("%s.log", format(Sys.time(), "%Y-%m-%d_%H-%M-%S"))
)

init_logging <- function(file) {
  logger::log_appender(logger::appender_file(file))
  logger::log_layout(logger::layout_glue_colors)  # pretty colours in console
...
# 🔑  make the helpers globally visible
for (f in c("log_debug","log_info","log_warn","log_error"))
  assign(f, get(f, inherits = FALSE), envir = .GlobalEnv)

log_info("AIR Tool startup – log initialised.")

# send all console output from this point on to the log file
log_con <- file(log_file, open = "a")
sink(log_con) # stdout
sink(log_con, type = "message") # stderr / messages / warnings

on.exit({
  sink(type = "message")    # restore when the session ends
  sink()
  close(log_con)
}, add = TRUE)

# Define the log directory and calculate the time threshold (30 days ago)
...
processResults <- function(settings,
  ...
  log_debug("Settings: {line}",
    line = paste(c(settings, AIRHome, tv_threshold, ov_threshold, model_yn,  model_ate), collapse = ", "))
  ...
  if (model_yn == "No") {
    ...
    ate_df_ref <- dplyr::filter(ate_df_all,
                                algorithm == "Stacked Super Learner")
...
# ──────────────────────────────────────────────────────────────
get_histogram_x <- function(df, xvar, threshold) {

  v       <- df[[xvar]]
  treated <- v >= threshold                    # single comparison rule
  ...
  colors <- c(Treated   = "#5D9AFF",
              Untreated = "#EAE1D7")

  untreated_label <- paste0("Untreated (< ", signif(threshold, 4), ")")
  treated_label   <- paste0("Treated (≥ ",  signif(threshold, 4), ")")
...
# ──────────────────────────────────────────────────────────────
get_histogram_y <- function(df, yvar, threshold) {
...
get_updated_graph <- function(dot, xvar, yvar, Zvars = NULL) {
  ...
  # 1 ── split DOT file into individual lines ------------------------------
  lines <- strsplit(dot, "\n", fixed = TRUE)[[1]]

  # helper to write a fresh node-declaration line
  make_decl <- function(node, colour) {
    sprintf('  "%s" [style=filled, fillcolor="%s"];', node, colour)
  }

  # 2 ── build the highlight lists -----------------------------------------
  nodes <- list(
    "#FFC107" = c(xvar, yvar),                       # treatment & outcome
    "#9394A2" = Zvars$Z[Zvars$grp == "Z1"],
    "#D4C7C7" = Zvars$Z[Zvars$grp == "Z2"]
  )

  # 3 ── drop any existing declarations of those nodes ---------------------
  pat <- sprintf('^\\s*"(%s)"\\s+\\[', paste(unlist(nodes), collapse = "|"))
  lines <- lines[!grepl(pat, lines)]

  # 4 ── insert new declarations right after “digraph g {” -----------------
  insert_at <- 2L                               # line index to insert after
  new_lines <- unlist(Map(make_decl, nodes, names(nodes)))
  lines <- append(lines, new_lines, after = insert_at - 1L)

  # 5 ── stitch everything back together and return ------------------------
  dot_out <- paste(lines, collapse = "\n")
  return(dot_out)
}
...
model_in <- reactive({
  ...
  shiny::validate(
    ...
    shiny::need(file.size(input$model_in$datapath) < 100*1024^2,
  ...
  log_debug("Upload received: {b} → {c}  (ext = {d})", b=input$model_in$name, c=input$model_in$datapath, d=ext)
  tryCatch({
    ...
    shiny::validate(
                    shiny::need(is.function(tryCatch(predict(mdl, head(mtcars)), error = function(e) NULL)),
           "Loaded object does not appear to have a usable predict() method."),
                    shiny::need(is.numeric(tryCatch(predict(mdl, head(df() %||% mtcars)),
                               error = function(e) NA)),
           "Model must return numeric predictions.")
    )
...
df <- reactive({
  req(input$file1)
  shiny::validate(
    shiny::need(tolower(tools::file_ext(input$file1$name)) == "csv",
                "Data file must be .csv"),
    shiny::need(file.size(input$file1$datapath) < 10*1024^2,
                "CSV too large (10 MB limit)")
  )

  tryCatch({
    df <- read_csv(input$file1$datapath, col_types = cols(.default = col_number()))
    log_debug("Read in data file: {msg}", msg = input$file1$datapath)
...
highlighted_graph <- reactive({
  ...
  if (!is.null(vars_sel()$x) && nzchar(vars_sel()$x))
    dot <- change_node_color(dot, vars_sel()$x, "'#FFC107'")

  ## — colour the outcome variable if one is chosen
  if (!is.null(vars_sel()$y) && nzchar(vars_sel()$y))
    dot <- change_node_color(dot, vars_sel()$y, "'#FFC107'")

  ## — colour adjustment sets only when they exist
  if (exists("Zvars", inherits = FALSE) && nrow(Zvars)) {
    if (any(Zvars$grp == "Z1"))
...
observeEvent( vars_ready(), ignoreInit = TRUE, {
  req(both_ready())
  req(graph_complete())

  # ── NEW: Y must be reachable from X to keep Tetrad happy
  shiny::validate(
                  shiny::need(vars_sel()$x != vars_sel()$y,
         "X and Y must be different columns"),
                  shiny::need(vars_sel()$y %in% get_X_descendents(vars_sel()$x, graphtxt_buffer),
  ...
  output$blankGraph <- renderGrViz({
    req(graph_complete())
    grViz( highlighted_graph() )   # highlighted_graph() already reacts
  })
})
...
observeEvent(input$goButton, {
  ...
  tryCatch({
    ...
    withProgress(message = 'Building Causal Graph', style = "notification", value = 0.1, {
      ...
      output$blankGraph <- renderGrViz({
        dot <- get_final_graph(rv$dot, vars_sel()$x, vars_sel()$y, Zvars)
        grViz(dot)
      })
...
output$xvar = renderUI({
  req(graph_complete())
  selected_val <- if (locked()) isolate(vars_sel()$x) else ""
  tagList(
    selectInput('xvar', 'Experimental (X) variable:', c("", names(df())),
                selected = selected_val),
    if (locked()) tags$script(HTML("
      var xvarDisableTries = 0;
      var disableXVar = setInterval(function() {
        var $sel = $('#xvar');
        var $inp = $('#xvar-selectized');
        if ($sel.length && $inp.length && $inp.parent().hasClass('selectize-input')) {
          $sel.prop('disabled', true);
          $inp.prop('disabled', true);
          $inp.parent().css({'pointer-events':'none', 'opacity':0.5});
          clearInterval(disableXVar);
        }
        xvarDisableTries++;
        if (xvarDisableTries > 10) clearInterval(disableXVar);
      }, 100);
    "))
  )
})
...
output$yvar = renderUI({
  req(x_ready())                 # need a valid X first
  req(graph_complete())

  shiny::validate(
    shiny::need(length(get_X_descendents(vars_sel()$x, graphtxt_buffer)) > 0,
                "Graph not ready yet – build the causal graph first")
  )

  xcol <- isolate(vars_sel()$x)  # freeze X so this block ignores Y changes

  choices <- setdiff(get_X_descendents(xcol, graphtxt_buffer), xcol)

  selected_val <- isolate(input$yvar)   # keep current Y, don’t depend on it

  # ensure current choice stays in the list even if not a descendant
  choices <- union(selected_val, choices)

  tagList(
    selectInput('yvar', 'Outcome (Y) variable:', c("", choices),
                selected = selected_val),
    if (locked()) tags$script(HTML("
      var yvarDisableTries = 0;
      var disableYVar = setInterval(function() {
        var $sel = $('#yvar');
        var $inp = $('#yvar-selectized');
        if ($sel.length && $inp.length && $inp.parent().hasClass('selectize-input')) {
          $sel.prop('disabled', true);
          $inp.prop('disabled', true);
          $inp.parent().css({'pointer-events':'none', 'opacity':0.5});
          clearInterval(disableYVar);
        }
        yvarDisableTries++;
        if (yvarDisableTries > 10) clearInterval(disableYVar);
      }, 100);
    "))
  )
})
...
output$ui_threshold_x <- renderUI({
  req(vars_sel()$x)
  vec <- df()[[vars_sel()$x]]
  val <- if (locked() && !is.null(input$tv_threshold))
           isolate(tv_val()) else default_split(vec)
  step <- fine_step(vec)  # half a bin, n_bins = 30
  tagList(
    tags$div(
      style = "display:flex;align-items:center;flex-wrap:wrap;",
      tags$span(paste(vars_sel()$x, "is considered treated when it is ≥")),
...
output$ui_threshold_y <- renderUI({
  ...
  tagList(
    tags$div(
      style = "display:flex;align-items:center;flex-wrap:wrap;",
      tags$span(paste(vars_sel()$y, "is considered success when it is ≥")),
...
output$download_report <- downloadHandler(
  ...
  content = function(file) {
    ...
    tryCatch({
      ...
      # ── generate images ────────────────────────────────────────────────────
      save_plot_pdf(
        get_histogram_x(vals$df, vals$xvar, vals$tv_threshold),
        file.path(tmpdir, "xhist.pdf")
      )
      save_plot_pdf(
        get_histogram_y(vals$df, vals$yvar, vals$ov_threshold),
