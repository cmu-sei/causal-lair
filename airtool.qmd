...
output$xvar <- renderUI({
  ...
  spinners <- get_spinsters(graphtxt_buffer)   # (aka spinsters)

  # X must be in df, not an orphan, and not a spinster (must have at least one parent and one child)
  choices <- setdiff(all_cols, union(orphans, spinners))
  choices <- sort(unique(choices))

  # selected value: keep prior if locked, else blank
  selected_val <- if (isTRUE(locked())) isolate(vars_sel()$x) else ""

  # normalize selected (must be in choices or "")
  ...
  tagList(
    ...
    if (isTRUE(locked())) tags$script(HTML("
      var tries = 0;
...
output$yvar <- renderUI({
  ...
  # compute descendants once
  descs <- get_X_descendents(xcol, graphtxt_buffer)
  descs <- setdiff(unique(descs), xcol)        # Y cannot be X
  descs <- sort(descs)

  # if graph isn't ready for X, show the validation message
  shiny::validate(
    shiny::need(length(descs) > 0,
                "Graph not ready yet – build the causal graph first")
  )

  # keep current Y if present
  selected_val <- isolate(input$yvar)
  if (is.null(selected_val)) selected_val <- ""

  # ensure current Y remains in list if it's already chosen
  if (nzchar(selected_val) && !(selected_val %in% descs)) {
    choices <- c(selected_val, descs)
  } else {
    choices <- descs
  }
