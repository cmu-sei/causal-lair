...
get_ui_interpretation <- function(AIRHome, df_vars, Zvars) {
  ...
  # --- special message when no overlap exists ---
  if (!overlap_exists) {
    return(paste0(
      "<strong>Consensus Could Not Be Reached for This Scenario.</strong> ",
      "The two causal analyses do not agree within their confidence ranges. ",
      "This suggests that the data or model assumptions may not support a stable or consistent causal conclusion for this scenario."
    ))
  }

  # --- continue with standard interpretation logic ---
  ...
  if ((all(dfr$z1_ATE_UCI < dfr$z2_ATE) & all(dfr$z2_ATE_LCI > dfr$z1_ATE)) |
    ...
    interpretation <- "The causal estimates appear inconsistent, suggesting there may not be enough information to train a reliable model."
  } else if (all(dfr$flag > dfr$zmin & dfr$flag < dfr$zmax)) {
    interpretation <- "Your model’s predictions align with the causal estimates. This indicates healthy model performance."
  } else if (all(dfr$flag > dfr$zmax) | all(dfr$flag < dfr$zmin)) {
    interpretation <- "Your model’s predictions diverge from the causal estimates, suggesting potential reliability issues. Investigate possible causes for this mismatch."
  } else {
    interpretation <- "Your model partly agrees with the causal estimates but shows inconsistencies. Interpret these results with caution."
  }

  # --- determine relative bias direction ---
  inZ1 <- any(between(dfr$flag, dfr$z1_ATE_LCI[1], dfr$z1_ATE_UCI[1]), na.rm = TRUE)
  inZ2 <- any(between(dfr$flag, dfr$z2_ATE_LCI[1], dfr$z2_ATE_UCI[1]), na.rm = TRUE)
  ...
  # --- simplified, human-readable result narrative ---
  result_text <- paste0(
    "Your model appears to be ", effect_estimation, " the effect that ",
    df_vars[1, ][[2]], " has on ", df_vars[2, ][[2]], effect_percent, ". ",
    "Based on the causal analysis, ", df_vars[1, ][[2]], 
    " is expected to have ", flagdir, " effect on ", df_vars[2, ][[2]], ". ",
    "As ", df_vars[1, ][[2]], " changes from baseline to experimental values,
    the likelihood of ", df_vars[2, ][[2]], " ",
    case_when(
      flagdir == "a negative" ~ paste0("decreases by about ",
        round(min(abs(minflag), abs(maxflag)), 2) * 100, "–",
        round(max(abs(minflag), abs(maxflag)), 2) * 100, "%."),
      flagdir == "a positive" ~ paste0("increases by about ",
        round(min(abs(minflag), abs(maxflag)), 2) * 100, "–",
        round(max(abs(minflag), abs(maxflag)), 2) * 100, "%."),
    ...
    case_when(
      ...
      TRUE ~ "biased"
    ), " results. Your model's effect is: ",rv$model_ate,". Overall, this suggests your seeing ",
    case_when(
      effect_estimation == "underestimating" ~ "a smaller than expected ",
      effect_estimation == "overestimating" ~ "a larger than expected ",
      TRUE ~ "an appropriate "
    ),
    "change in ", df_vars[2, ][[2]], " as ", df_vars[1, ][[2]], " varies. ",
    case_when(
      inZ1 & inZ2 ~ "No evidence of bias is detected at this time.",
      inZ1 == TRUE & inZ2 == FALSE ~ paste0(
        "Bias may be introduced through variable(s): ",
        paste0(Zvars$Z[2], collapse = ", "), " (see causal graph)."
      ),
      inZ2 == TRUE & inZ1 == FALSE ~ paste0(
        "Bias may be introduced through variable(s): ",
        paste0(Zvars$Z[1], collapse = ", "), " (see causal graph)."
      ),
      TRUE ~ paste0(
        "Bias may be introduced through variable(s): ",
          case_when(
            identical(Zvars$Z[1], Zvars$Z[2]) ~ Zvars$Z[1],
            TRUE ~ paste0(paste0(Zvars$Z[1], collapse = ", "), " and/or ", paste0(Zvars$Z[2], collapse = ", ")) 
          ),
          " (see causal graph)."
      )
    )
  ...
  # --- improved, cleaner disclaimer text ---
  results_text <- paste0(
    result_text, "\n These treatment effect estimates are based on the variables included in your causal graph. ",
    "If important factors are missing, the results may not represent true causal relationships. ",
    "The system assumes that any missing variables are independent of those included, which may not hold in practice. ",
    "Interpret these estimates carefully and consider updating your graph if key relationships are uncertain. "
    )
