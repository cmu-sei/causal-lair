...
output$ui_interpretation = renderUI({
  ...
  flagdir <- case_when(maxflag < 0 ~ "a negative",
                       ...
                       TRUE ~ "no")
  effect_estimation <- case_when(any(abs(flag) < min(abs(maxflag), abs(minflag))) ~ "underestimating",
                                 any(abs(flag) > max(abs(maxflag), abs(minflag))) ~ "overestimating",
                                 TRUE ~ "correctly estimating")
  effect_percent <- case_when(effect_estimation == "underestimating" ~ paste0(" by ", round(abs(maxflag) - abs(mean(flag)), 2)*100, "-",round(abs(minflag) - abs(mean(flag)), 2)*100, "%"),
                          effect_estimation == "overestimating" ~ paste0(" by ", round(abs(mean(flag)) - abs(minflag), 2)*100, "-",round(abs(mean(flag)) - abs(maxflag), 2)*100, "%"),
                          TRUE ~ "")
