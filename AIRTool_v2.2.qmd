...
output$ui_interpretation = renderUI({
  ...
  flagdir <- case_when(maxflag < 0 ~ "a negative",
                       ...
                       TRUE ~ "no")
  effect_estimation <- case_when(any(abs(min(dfr$flag)) < min(abs(maxflag), abs(minflag))) ~ "underestimating",
                                 any(abs(max(dfr$flag)) > max(abs(maxflag), abs(minflag))) ~ "overestimating",
