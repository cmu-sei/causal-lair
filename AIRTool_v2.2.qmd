  ...
  ### AIR Step 1 ----------------
    if (Sys.info()["sysname"] == "Linux") {
      ...
      graphlist <- AIR_getGraph(df(), knowledge()) # prod
  	  ...
  	  ts <- graphlist[[2]]
  	  MC_passing_cpdag_already_found <- graphlist[[3]]
  	  best_cpdag_seen_so_far <- graphlist[[4]]
  	  ### this is the problem -----
  	  # this file isn't being formatted properly. I need to actually run graphToDot to fix
  	  # dot <- .jcall("edu/cmu/tetrad/graph/GraphSaveLoadUtils", "Ljava/lang/String;", "graphToDot", graphtxt) # prod
