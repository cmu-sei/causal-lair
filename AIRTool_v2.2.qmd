...
cat("Begin Error Logging\n", file = "error_log.txt", append = FALSE)
...
observeEvent(input$goButton, {
    ...
    tryCatch({
        ...
        registerDoParallel(cluster)
          ...
          cat(errMsg, file = "error_log.txt", sep = "\n", append = TRUE)
          NULL
        })
    ...
    foreach(k = 1:nrow(combos), 
            ...
            .export = c("Zvars", "runSuperLearner", "AIRHome", "tv_dir_val", "tv_threshold_val", "ov_dir_val", "ov_threshold_val", "df_vars", "processResults", "model_yn")) %dopar% {
        i <- combos[k, "i"]
        j <- combos[k, "j"]
        ...
        tryCatch({
          runSuperLearner(settings, AIRHome, tv_dir_val, tv_threshold_val, ov_dir_val, ov_threshold_val)
    ...
    for (i in unique(Zvars$name)) {
      for (j in unique(Zvars$grp)) {
        settings <- data.frame(doc_title = paste0(i,"-",j),
                               ...
                               confounders = paste0(Zvars[Zvars$name == i & Zvars$grp == j,]$Z, collapse = " "))
        tryCatch({
          processResults(settings, AIRHome, tv_dir_val, tv_threshold_val, ov_dir_val, ov_threshold_val, model_in(), model_yn, model_ate)
          }, error = function(e) {
