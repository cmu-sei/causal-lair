...
observeEvent(input$goButton, {
    ...
    for (i in unique(Zvars$name)) {
      for (j in unique(Zvars$grp)) {
        ...
        # runSuperLearner(settings, AIRHome, tv_dir, tv_threshold)
      }
    }
    
    incProgress(0.2, message = "Processing Results")
    processResults(settings, AIRHome, input$tv_dir, input$tv_threshold, input$ov_dir, input$ov_threshold)
...
output$ui_interpretation = renderUI({
  ...
  if (flag >= dfr$z1_ATE_LCI & flag <= dfr$z1_ATE_UCI) {
    inZ1 <- TRUE
  } else { inZ1 <- FALSE }
  if (flag >= dfr$z2_ATE_LCI & flag <= dfr$z2_ATE_UCI) {
    inZ2 <- TRUE
  } else { inZ2 <- FALSE }
  ...
  flagdir <- case_when(maxflag < 0 ~ "a negative",
                       ...
                       TRUE ~ "no")
  effect_estimation <- case_when(abs(flag) < min(abs(maxflag), abs(minflag)) ~ "underestimating",
                                 abs(flag) > max(abs(maxflag), abs(minflag)) ~ "overestimating",
                                 TRUE ~ "correctly estimating")
  effect_percent <- case_when(effect_estimation == "underestimating" ~ paste0(" by ", round(abs(maxflag) - abs(flag), 2)*100, "-",round(abs(minflag) - abs(flag), 2)*100, "%"),
                          effect_estimation == "overestimating" ~ paste0(" by ", round(abs(flag) - abs(minflag), 2)*100, "-",round(abs(flag) - abs(maxflag), 2)*100, "%"),
                          TRUE ~ "")
