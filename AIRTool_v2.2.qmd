...
observeEvent(input$updateButton, {
  tryCatch({
    ...
    ## inside the observer / reactive that updates the variable choices
    df_vars <- data.frame(
      var = c("TV", "OV"),
      val = c(input$xvar, input$yvar),   # <- current selections
      stringsAsFactors = FALSE
    )

    ## make it available everywhere that still relies on the global
    ...
    tryCatch({
      ...
      save_dot_to_pdf(
        DiagrammeR::grViz(                       # << wrap the DOT string
          get_updated_graph(graph_snapshot,
                            vals$graph_update,
