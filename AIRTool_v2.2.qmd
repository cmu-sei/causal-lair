...
observeEvent(input$buildButton, {
  ...
  if (!is.na(knowledge())){
    ...
    sendSweetAlert(
      session = shiny::getDefaultReactiveDomain(),,
      title = "Missing Knowledge File",
      text = "Knowledge file is missing or corrupt. Please upload a valid knowledge file.",
      type = "fail",
      btn_labels = "Continue",
      btn_colors = "#3085d6",
      closeOnClickOutside = TRUE,
...
observeEvent(input$goButton, {
    ...
    foreach(i = unique(Zvars$name)) %dopar% {
      foreach(j = unique(Zvars$grp)) %dopar% {
